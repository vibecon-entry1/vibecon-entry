...
> **STATUS: the round 1-3 art winners below are SUPERSEDED.** They were
> rejected at user review as too timid. The active art track is the direction
> sheets in `assets-wow/directions/` (see CRITIQUE_LOG round 4); a user
> direction pick gates all further art work. The SFX section remains VALID
> and unaffected. The sections below are kept for the pipeline facts, which
> still apply to whichever direction wins.
...
`assets-wow/raw/<name>/` (prompt.txt + raw_0.jpg), and every derived sheet is
a pure function of those raws via `tools/genart_v2_post.py` (crop → LANCZOS
downscale to 640x360 → median-cut quantize to a 36-40 color budget → sprite
compositing at a measured ground line). Divergence + palette numbers per
sheet: `assets-wow/directions/DIVERGENCE.json`.
